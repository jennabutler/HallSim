// HallSim.cpp : Defines the entry point for the console application.
//
#include "stdafx.h"
#include "Cell.h"
...
void WriteToFile(std::list<Cell *>::const_iterator iterator, int time, std::list<Cell *>::const_iterator iteratorEnd, string paramSet, int it){
	...
	if (time == -1)
		fileName << "..\\Output\\Feb_5_2014_1\\Cells_" << paramSet <<"_01_2_" << RUN << "_it_" << it << "_end.txt";
	else
		fileName << "..\\Output\\Feb_5_2014_1\\Cells_" << paramSet <<"_01_2_" << RUN << "_" << time << "_it_" << it <<".txt";
...
void RunSimulation(Combo c, int it){
	...
	while (events.size() > 0 && counter<42000){
		//Get next event
	...
	//	Write out every x timesteps
		...
		//if (counter % 30000 == 0){
			...
			//Every 2000 runs, check if the cells at the center are alive or dead? Mark? TO-DO
